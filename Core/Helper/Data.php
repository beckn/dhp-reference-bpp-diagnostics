...
class Data extends AbstractHelper
{
    ...
    const PERSON_IMAGE_DIR = "person/images/";
    ...
    {
        $bodyRequest = $this->_restRequest->getBodyParams();
        $this->_logger->info("Received request here => ");
        $this->_logger->info(json_encode($bodyRequest));
...
//        }
     ...
     * @return bool
     */
    public function addAgentCondition($personName){
        $bodyRequest = $this->_restRequest->getBodyParams();
        if(isset($bodyRequest["message"]["intent"]["fulfillment"]["agent"]["name"])){
            $agentName = $bodyRequest["message"]["intent"]["fulfillment"]["agent"]["name"];
            $matchPercentage = similar_text($agentName, $personName, $perc);
