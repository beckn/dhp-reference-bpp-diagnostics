...
class UpgradeData implements UpgradeDataInterface
{
    ...
    const ITEM_FULFILLMENT_OPTIONS = 'item_fulfillment_options'.InstallData::SUFFIX;
    ...
    public function upgrade(ModuleDataSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '1.1.4', '<')) {
            $attribute = $this->_eavConfig->getAttribute(Product::ENTITY, self::ITEM_FULFILLMENT_OPTIONS);
            $attributeCheck = ($attribute && $attribute->getId());
            if(!$attributeCheck){
                $eavSetup = $this->_eavSetupFactory->create(['setup' => $setup]);
                /**
                ...
                $eavSetup->addAttribute(
                    \Magento\Catalog\Model\Product::ENTITY,
                    self::ITEM_FULFILLMENT_OPTIONS,
                    [
                        'type' => 'varchar',
                        'backend' => 'Magento\Eav\Model\Entity\Attribute\Backend\ArrayBackend',
                        'frontend' => '',
                        'label' => 'Item Fulfillment Options',
                        'group' => 'Beckn Configuration',
                        'input' => 'multiselect',
                        'class' => '',
                        'source' => 'Beckn\Core\Model\Config\Source\ItemFulfillmentOptions',
                        'global' => \Magento\Eav\Model\Entity\Attribute\ScopedAttributeInterface::SCOPE_GLOBAL,
                        'visible' => true,
                        'required' => false,
                        'user_defined' => true,
